...
function buildImplementationTaskTemplateBlock({
  ...
  const lines = [
    ...
    "- inspect the current workspace structure first when needed",
    "- prefer the smallest responsible existing file(s)",
    "- follow the detected stack and existing project structure",
    "- after routine inspection, continue with the implementation when the request is clear",
    "- do not stop after routine inspection just to ask for permission to proceed",
    "- do not ask which file or directory to use unless the request is genuinely ambiguous or multiple existing targets are equally plausible",
    "- for ordinary feature requests, choose the simplest existing structure that fits and continue",
...
function buildNoProjectTaskTemplateBlock({ projectOpen }) {
  ...
  return [
    ...
    "- recommend one coherent manual path before giving setup steps when the user has no stack preference",
    "- keep the chosen stack and the setup steps consistent with each other",
    "- prefer a normal scaffolded setup path over hand-writing starter boilerplate unless the user explicitly asks for a by-hand setup",
    ...
    "- once a manual path is chosen, give only the steps that match that path",
...
function buildEmptyFolderTaskTemplateBlock({ projectOpen, tree }) {
  ...
  return [
    ...
    "Treat this as an open workspace with no current app or project files to modify yet.",
    "For implementation-style requests in this empty folder:",
    "- explain that the folder is empty and there is no current app to modify",
    "- do not treat the request as normal in-project implementation work yet",
    "- do not ask which file or directory to use",
    "- do not invent ad-hoc app files as though an existing project already exists",
    "- prefer Preview -> Generate for a supported starter template project",
    "- only switch into manual setup guidance if the user explicitly asks to bypass KForge or asks for manual steps",
    "For new app or starter requests in this empty folder:",
    "- prefer Preview -> Generate for a supported template project",
    "- do not describe this path as Services -> Preview -> Generate",
    "- because a folder is already open, do not tell the user to open or create a folder first",
    ...
    "- if the user does not know which template to choose, recommend the best supported app starter for the request before sending them to Preview -> Generate",
    "- for a general app request, do not default to a static-site starter unless the user explicitly wants a simple static site",
    "",
  ].join("\n");
}
