...
pub struct OpenAIResponsesRequest {
    ...
    /// Privacy-first: Responses are stored by default; disable by setting store:false. :contentReference[oaicite:1]{index=1}
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Responses API supports max_output_tokens. (Name per API/docs examples.)
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub enum OpenAIOutputItem {
    ...
    // There are other types (reasoning, tool calls, etc). We ignore them for v1. :contentReference[oaicite:2]{index=2}
    #[serde(other)]
    Other,
}
...
/// Extract all assistant output_text segments (robust against multiple output items). :contentReference[oaicite:3]{index=3}
pub fn extract_output_text(resp: &OpenAIResponsesResponse) -> String {
    let mut out = String::new();
    let Some(items) = &resp.output else { return out };
