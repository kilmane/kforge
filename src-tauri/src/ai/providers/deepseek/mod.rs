...
impl DeepSeekProvider {
  ...
  fn resolve_base_url(&self, req: &AiRequest) -> String {
    let raw = req.endpoint.clone().unwrap_or_else(|| self.base_url.clone());
  ...
  fn map_provider_error(&self, err: ProviderError) -> AiError {
    match err {
      ...
      ProviderError::Json(e) => AiError::new(self.provider_id(), AiErrorKind::Parse, e.to_string()),
  ...
  fn extract_usage(v: &Value) -> Option<AiUsage> {
    ...
    let prompt_tokens = u.get("prompt_tokens").and_then(|x| x.as_u64()).map(|n| n as u32);
    let completion_tokens = u
      .get("completion_tokens")
      .and_then(|x| x.as_u64())
      .map(|n| n as u32);
    let total_tokens = u.get("total_tokens").and_then(|x| x.as_u64()).map(|n| n as u32);
  ...
  fn build_chat_completions_body(req: &AiRequest) -> Value {
    ...
    if let Some(mt) = req.max_output_tokens {
      obj.insert("max_tokens".to_string(), Value::Number(serde_json::Number::from(mt)));
    }
