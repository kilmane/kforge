...
fn run_stripe_setup(app: &AppHandle, project_dir: &PathBuf) -> Result<(), String> {
    ...
    let has_complete_env_pair = has_secret_key && has_publishable_key;
    let env_file_exists = project_dir.join(".env").exists();
    ...
    emit_log(
        app,
        "status",
        "Stripe configuration looks good. This project has the core payment keys available.",
    );
    emit_log(
        app,
        "stdout",
