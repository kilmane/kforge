...
fn ensure_supabase_env_example(project_dir: &PathBuf) -> Result<bool, String> {
    ...
    if env_example_path.exists() {
        ...
        let has_supabase_url = has_env_line("SUPABASE_URL");
        let has_supabase_anon_key = has_env_line("SUPABASE_ANON_KEY");
        let has_vite_supabase_url = has_env_line("VITE_SUPABASE_URL");
        let has_vite_supabase_anon_key = has_env_line("VITE_SUPABASE_ANON_KEY");

        if has_supabase_url
            && has_supabase_anon_key
            && has_vite_supabase_url
            && has_vite_supabase_anon_key
        {
            return Ok(false);
        }
        ...
        next.push_str("# Supabase environment template generated by KForge\n");
        next.push_str("# Use anon/public keys in frontend projects.\n");
        next.push_str("# Do not put service-role, private, or admin keys in frontend env files.\n");

        if !has_supabase_url {
            next.push_str("SUPABASE_URL=\n");
        }
        if !has_supabase_anon_key {
            next.push_str("SUPABASE_ANON_KEY=\n");
        }
        if !has_vite_supabase_url {
            next.push_str("VITE_SUPABASE_URL=\n");
        }
        if !has_vite_supabase_anon_key {
            next.push_str("VITE_SUPABASE_ANON_KEY=\n");
        }
    ...
    let content = [
        ...
        "#",
        "# Use anon/public keys in frontend projects.",
...
fn run_supabase_setup(app: &AppHandle, project_dir: &PathBuf) -> Result<(), String> {
    ...
    if created_env_example {
        emit_log(
            app,
            "stdout",
            "Created or updated .env.example with Supabase connection placeholders.",
        );
    } else {
        emit_log(
            app,
            "stdout",
            ".env.example already contains Supabase placeholders. Leaving it unchanged.",
        );
    }
    if supabase_config_exists {
...
pub fn supabase_quick_connect(app: AppHandle, project_path: String) -> Result<(), String> {
    ...
    let result = (|| -> Result<(), String> {
        ...
        emit_log(&app, "status", "Supabase quick connect finished. Starter setup is ready; connection values may still need to be added or verified.");
        if supabase_config_exists {
            emit_log(
                &app,
                "stdout",
